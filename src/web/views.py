...
def recommend(request):
	...
	book_list=list(Book.objects.filter(id__in = pred_idxs_sorted,).order_by(preserved)[:10])
	return render(request,'web/recommend.html',{'book_list':book_list})
...
def detail(request,book_id):
	...
	if request.method == "POST":
		...
		ratingObject.save()
		messages.success(request,"Rating is successfull✔")
		return redirect("index")
	return render(request,'web/detail.html',{'books':books})
...
def Login(request):
	...
	if request.method=="POST":
		...
		if user is not None:
			...
			print("THREE")
			return render(request,'web/login.html',{'error_message': 'Invalid login credentials'})
	print("FOUR")
